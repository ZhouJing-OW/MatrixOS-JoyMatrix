...
class UI4pxNoteName : public UIComponent {
 public:
  ...
  UI4pxNoteName(int8_t* note = nullptr, Color color1 = COLOR_YELLOW, Color color2 = COLOR_WHITE, Color color3 = COLOR_AZURE) {
    this->color1 = color1;
    this->color2 = color2;
    this->color3 = color3;
    this->note = note;
  ...
  void Render4px(Point origin, Color color, char name) {
    for (int8_t x = 0; x < 3; x++)
    {
      ...
      { MatrixOS::LED::SetColor(origin + Point(x, 3 - y), bitRead(narrowFont4[(int)name - 32][x + 1], y) ? color : COLOR_BLANK); }
    }
  }
  ...
  void RenderSymbol(Point origin, Color color, bool isSharp, bool isNagative) 
  {
    for(int8_t x = 0; x < 2; x++)
    {
      for(int8_t y = 0; y < 2; y++)
        MatrixOS::LED::SetColor(origin + Point(x, y), isSharp ? color : COLOR_BLANK);
      MatrixOS::LED::SetColor(origin + Point(x, 2), COLOR_BLANK);
      MatrixOS::LED::SetColor(origin + Point(x, 3), isNagative ? color : COLOR_BLANK);
    }
  }
