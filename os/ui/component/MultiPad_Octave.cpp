...
{ 
  ...
  Color color1 = color.Scale((int8_t)((float)channelConfig->octave[channel] * 239/ 9 + 16));
  Color color2 = color.Scale((int8_t)((1 - (float)channelConfig->octave[channel]) * 239 / 9 + 16));
...
bool MultiPad::OctaveShiftKeyEvent(Point xy, KeyInfo* keyInfo)
{
  ...
  { 
    int8_t *octave = &channelConfig->octave[channel];
