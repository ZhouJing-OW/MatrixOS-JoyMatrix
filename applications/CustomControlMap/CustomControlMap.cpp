...
void CustomControlMap::ActionMenu() {
  ...
  // brightnessBtn.OnPress([&]() -> void { MatrixOS::SYS::NextBrightness(); });
  ...
  reloadBtn.SetSize(Dimension(1, 1));
  reloadBtn.OnPress([&]() -> void { Reload(); });
  actionMenu.AddUIComponent(reloadBtn, Point(15, 3));
  ...
  actionMenu.AddUIComponent(passthroughControl, Point(0, 2));                                        
