...
void Performance::MidiEventHandler(MidiPacket midiPacket) {
  ...
  switch (midiPacket.status)
  {
    ...
    case SysExEnd:
      ...
      break;
    case Sync:
      MLOGD("Performance", "Midi Recived %d", midiPacket.status);
      break;
    default:
      break;
  }
...
int8_t Performance::XYToNote(Point xy) {
  switch (currentKeymap)
  {
    case 0:
    {
      if (xy.x >= 0 && xy.x < 16 && xy.y >= 0 && xy.y < 4)
  ...
  switch (sysExBuffer[0])
  {
    case 0x5f: //Apollo batch fill - https://github.com/mat1jaczyyy/lpp-performance-cfw/blob/0c2ec2a71030306ab7e5491bd49d72440d8c0199/src/sysex/sysex.c#L54-L120
    {
      ...
      while (ptr < sysExBuffer.size())
      {
        ...
        for (uint16_t n = 0; n < n_count; n++)
        {
          ...
          {
            Point xy = Point(sysExBuffer[ptr] % 16, 3 - (sysExBuffer[ptr] / 16));
    ...
    case 0x5e: //Apollo regular fill - https://github.com/mat1jaczyyy/lpp-performance-cfw/blob/0c2ec2a71030306ab7e5491bd49d72440d8c0199/src/sysex/sysex.c#L115
    {
      ...
      while (ptr < sysExBuffer.size())
      {
        ...
        {
          Point xy = Point(index % 16, 3 - (index / 16));
          MatrixOS::LED::SetColor(xy, color, targetLayer);
        }
...
void Performance::ActionMenu() {
  ...
  */
  ...
  // Note Pad
  UINotePad notePad(Dimension(16, 1), keymap_color[currentKeymap], keymap_channel[currentKeymap],
                    (uint8_t*)note_pad_map[currentKeymap]);
  actionMenu.AddUIComponent(notePad, Point(0, 3));
  ...
  actionMenu.AddUIComponent(menuLockBtn, Point(0, 0));  // Current the currentKeymap is directly linked to
  ...
  actionMenu.AddUIComponent(flickerReductionBtn, Point(1, 0));  // Current the currentKeymap is directly linked to
                                                                // compatibilityMode. Do we really need > 2 keymap tho?
  ...
  actionMenu.AddUIComponent(compatibilityModeBtn, Point(14, 0));  // Current the currentKeymap is directly linked to
