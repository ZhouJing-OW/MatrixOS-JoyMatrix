...
volatile gpio_num_t keypad_write_pins[8] = 
{
  GPIO_NUM_21,
  GPIO_NUM_14,
  GPIO_NUM_13,
...
volatile gpio_num_t keypad_read_pins[10] = 
{
  GPIO_NUM_1,
  GPIO_NUM_2,
  GPIO_NUM_4,
...
int main(void)
{
  ...
  while(true)
  {
    for (uint8_t x = 0; x < 8; x++)
    {
      ...
      for (uint8_t y = 0; y < 10; y++)
      {
        result[x + (8 * (y % 2 ))][y / 2] = ulp_riscv_gpio_get_level(keypad_read_pins[y]); // 8 x 10 to 16 x 5
      };
